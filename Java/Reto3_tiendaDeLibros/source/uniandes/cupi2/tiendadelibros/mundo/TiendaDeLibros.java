...
public class TiendaDeLibros
{
    ...
    {	
    	...
    	for (Libro actual : catalogo) {
    		double precio = actual.darPrecioVenta();
    		
    		for (int i=0; i <descuentos.length ; i++) {
    			if (precio >= lim_inf[i] && precio < lim_sup[i]) {
    				actual.precioconDescuento(descuentos [i]);
    			}
    		}
    	}
    ...
    public Transaccion metodo2( )
    {
    	...
    	for (Libro actuallibro: catalogo) {
    		
    		for(Transaccion actualtransaccion : actuallibro.darTransacciones()) {
    			if(actualtransaccion.darTipo().equals(Transaccion.Tipo.VENTA)) {
    				double valor_transaccion = actuallibro.darPrecioVenta()*actualtransaccion.darCantidad();
    				if(valor_transaccion > valor) {
    					valor = valor_transaccion;
    					mascostosa = actualtransaccion;
    				}
    			}
    		}
    	}
