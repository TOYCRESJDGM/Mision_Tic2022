...
public class SimuladorBancario
{
    ...
    public void invertirCDT( double pMonto, String pInteresMensual ) throws Exception
    {
		...
		consecutivo ++;
		Transaccion nueva = new Transaccion(consecutivo,pMonto, Transaccion.Tipo.ENTRADA, Transaccion.Cuenta.CDT);
    ...
    public void avanzarMesSimulacion( )
    {
        ...
        if(inversion.darValorInvertido()>0){
        	consecutivo ++;
        	double invertirMeses =  inversion.darValorInvertido() * inversion.darInteresMensual();
    		Transaccion nueva = new Transaccion(consecutivo,invertirMeses, Transaccion.Tipo.ENTRADA, Transaccion.Cuenta.CDT);
    		transacciones.add(nueva);
        }
    }
    ...
    public void cerrarCDT( )
    {
        ...
        consecutivo ++;
		Transaccion nueva = new Transaccion(consecutivo,valorCierreCDT, Transaccion.Tipo.SALIDA, Transaccion.Cuenta.CDT);
		transacciones.add(nueva);
		
		consecutivo ++;
		Transaccion nueva_1 = new Transaccion(consecutivo,valorCierreCDT, Transaccion.Tipo.ENTRADA, Transaccion.Cuenta.CORRIENTE);
		transacciones.add(nueva_1);
    }
    ...
    public void pasarAhorrosToCorriente()
    {
		...
		consecutivo ++;
		Transaccion nueva_1 = new Transaccion(consecutivo,cantidad, Transaccion.Tipo.ENTRADA, Transaccion.Cuenta.CORRIENTE);
		transacciones.add(nueva_1);
    }
	...
	public int metodo3(int pTipo, int pCuenta) {
		...
		for(Transaccion transaccion: transacciones){
			if(transaccion.darTipoTransaccion() == pTipo){
				if(transaccion.darCuentaTransaccion() == pCuenta){
					double valor = transaccion.darValor();
					if (valor > valorMaximo){
						valorMaximo= valor;
						respuesta = transaccion.darConsecutivo();
					}
				}
			}
		}
	...
	public void verificarInvariante(){
		assert ahorros.darSaldo() + inversion.calcularValorPresente(mesActual) < INVERSION_MAXIMO : "ERROR: SE SUPERO EL MONTO MAXIMO DE INVERSIÓN";
		assert inversion != null: "El CDT es null";
		assert ahorros != null: "La cuenta de ahorros es null";
		assert corriente != null: "La cuenta corriente es null";
		assert consecutivo >=0: "La cuenta corriente es null";
	}
}