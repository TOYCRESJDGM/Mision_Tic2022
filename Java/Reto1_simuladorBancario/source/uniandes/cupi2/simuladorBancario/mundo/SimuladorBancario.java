...
public class SimuladorBancario
{
    ...
    {	
    	mesActual += 1;
    	double saldo_inicial = ahorros.darSaldo();
    	ahorros.actualizarSaldoPorPasoMes();
    	interesahorro += ahorros.darSaldo() - saldo_inicial;
    ...
    {
    	double valor_inicial_cdt = inversion.ValorInicial();
        double valorCierreCDT = inversion.cerrar( mesActual );
        interescdt += (valorCierreCDT-valor_inicial_cdt);
    ...
    {
    	double saldointeres =  ahorros.darSaldo()*Math.pow((1+ahorros.darInteresMensual()),pMeses);
    	double interes_ahorro = saldointeres - ahorros.darSaldo();
    	interesahorro += interes_ahorro; 
    	consignarCuentaAhorros(interes_ahorro);
    	mesActual += pMeses;
    	calcularSaldoTotal();  	
    ...
    {	
    	cerrarCDT();
    	double intereses = interescdt + interesahorro;
    	mesActual=1;  	
    	corriente =  new CuentaCorriente();
    	ahorros =  new CuentaAhorros();
    	inversion = new CDT();
