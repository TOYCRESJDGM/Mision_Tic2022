...
def correoInstitucional (nombre: str, apellido :str,documento:int)->str:   
    #Dos nombres
    numero =str(documento)
    if (' ' in nombre):
        espacio = nombre.index(' ')
        coma = apellido.index(',')
        correo= nombre[0]+ nombre[espacio +1]+'.'+ apellido[coma +2:] + numero[len(numero)-2:]
    #Un solo nombre   
    else:
        coma = apellido.index(',')
        correo = nombre[0]+apellido[coma+2]+'.'+ apellido[:coma] + numero[len(numero)-2:]
    correo = correo.lower() 
...
def facultades(info:dict)->list:
    
    facultades = set()
    for valor in info.values():
        
       asignaturas = valor['materias']
       i=0
       while i<= len(asignaturas)-1:
           
           facultad = asignaturas[i]['facultad']
           facultades.add(facultad)
           i+=1
           
    lista_facultades = list(facultades)
    return sorted(lista_facultades)

def correos(lista:list)->list:    
    correos = set ()
    ...
    return sorted (lista_correos)

def est_intercambio(codigo:str)->bool:
    intercambio = False
    periodo = codigo[4:6]
    if periodo == '05':
        intercambio= True
...
def promedio_facultades(info: dict, contando_externos : bool = True )->tuple:
    ...
    elementos = info.items()
    lista_facultades = facultades(info)
    lista_correos = []
    creditos_facultad = dict.fromkeys(lista_facultades,0)
    notas_facultad = dict.fromkeys(lista_facultades,0)
    
    
    for llave,valor in elementos:
        ...
        materia = valor['materias']        
        cantidad = len(materia)
        intercambio = est_intercambio(codigo)
            
        if cantidad == 0:
            continue
        for elementos in range(len(materia)):
                
            materias_creditos_0 = 0
            materias_retiradas = 0
            cant_intercambio = 0
            if contando_externos == True:                
                if materia[elementos]['creditos'] == 0 or materia[elementos]['nota']==0:
                       
                    materias_creditos_0 = materias_creditos_0 + 1
                else: 
                    if materia[elementos]['retirada']=='Si':
                        materias_retiradas = materias_retiradas + 1
                            
                    else:
                        try:
                            facultad = materia[elementos]['facultad']
                            creditos = materia[elementos]['creditos']
                            nota = materia[elementos]['nota']
                            valor = nota*creditos
                                
                            correo = correoInstitucional(nombres,apellidos,documento)
                            lista_correos.append(correo)
                                
                            if facultad in lista_facultades:
                                    
                                creditos_facultad[facultad] = creditos + creditos_facultad[facultad]
                                notas_facultad[facultad] = valor + notas_facultad[facultad]
                        except:
                            return 'Error numérico.'
            else:                
                if intercambio == True:
                    cant_intercambio = cant_intercambio +1
                else:
                    if materia[elementos]['creditos'] == 0 or materia[elementos]['nota']==0:
                          materias_creditos_0 = materias_creditos_0 + 1
                    else: 
                        if materia[elementos]['retirada']=='Si':
                            ...
                            letras_codigo_materia = materia[elementos]['codigo']
                            if letras_codigo_materia[0:4] == programa:
                                try:
                                    facultad = materia[elementos]['facultad']
                                    creditos = materia[elementos]['creditos']
                                    nota = materia[elementos]['nota']
    ...
    ordenar_correos=correos(lista_correos)    
    try:   
        resultado={}                  
        for facultad in lista_facultades:                             
            resultado.update({facultad :round(notas_facultad[facultad]/creditos_facultad[facultad],2) })
            resultado.update([])
    except:
        return 'Error numérico.'
    respuesta = (resultado,ordenar_correos)         
    return respuesta
